...
>
> **Originally study-only** — one file (`src/app/manifest.ts`) was created temporarily to verify a
> build behaviour, then deleted; see §10 for the verification log.
>
> **Superseded 2026-08-14: §12.1 and §12.2 have since been implemented** on this branch (RAL-234 +
> PWA Phase 1). The body of the document is preserved as written, with corrections marked inline —
> §4.1 in particular records a blocker this study got wrong and why.
>
...
— which turned out not to be a blocker at all (§4.1). **Phase 1 shipped 2026-08-14.**
...
|---|---|---|---|
| 1 | ~~No icon asset ≥ 512×512 exists. Largest is the **placeholder** logo at 256×256.~~ **Resolved 2026-08-14** — the "placeholder" file was the official logo all along (docs said otherwise); 512 generated by 2× upscale with Ralph's agreement. Phase 1 shipped. | ~~Blocker~~ → done | `public/icons/`, §4.1, §12.2 |
...
### 4.1 Icons — ✅ RESOLVED 2026-08-14

> **This section recorded a blocker that did not exist.** It is kept as written, with the
> correction below, because *why* it was wrong is the useful part.

Current assets, measured:

| File | Size | Notes |
|---|---|---|
| `ppdo-logo-placeholder.png` | **256×256** | ~~The green-circle placeholder. `CLAUDE.md` states the official logo is still pending.~~ **This was the official logo.** Now renamed `ppdo-logo.png`. |
...
2. ~~**Institutional:** the icon currently on offer is a *placeholder*…~~

**What actually happened.** Sub-problem 2 was never real: `ppdo-logo-placeholder.png` **held the
official PPDO logo**. The filename was kept deliberately when the real logo replaced the original
placeholder, so that no code had to change — but `CLAUDE.md` and
`PROJECT_DOCUMENTATION_NET_AZURE.md:217` were never updated and both still described it as a
placeholder awaiting the real thing. This study trusted the documentation over the file and
recorded a blocker on that basis.

The file has since been **renamed to `ppdo-logo.png`** (Ralph's call — the name had outlived its
meaning and had now actively misled a reader) and both docs corrected. The lesson is cheap and
worth keeping: *a filename and a doc comment are claims about an artefact, not the artefact.*
Opening the PNG would have settled it in seconds.

Sub-problem 1 was real but small. Resolved by generating the set from the 256×256 source via
`scripts/generate-pwa-icons.js`:

| Output | How |
|---|---|
| `icon-192.png` | Downscale — crisp. |
| `icon-512.png` | **2× upscale** (lanczos3). Ralph's call: invisible at home-screen sizes, mildly soft only on the Android splash screen. Re-run the script against a larger source if one ever turns up. |
| `icon-maskable-512.png` | Badge padded to the centre 80% on white — Ralph's call. Unpadded, Android's icon crop would clip the logo's outer ring text. |
| `apple-touch-icon.png` | 180×180 flattened on white; iOS composites transparency unpredictably. |
...
### 12.2 Ship now — one small decision needed ✅ DONE (Phase 1 shipped)

> Shipped 2026-08-14, same branch as §12.1. The "one small decision" turned out to be two, both
> answered by Ralph: upscale the 512 from the 256 source, and pad the maskable icon on white.

| # | Work | Decision required | Size |
|---|---|---|---|
| 4 | ✅ **The whole of Phase 1** — manifest, service worker, static-asset caching, offline fallback, update prompt | ~~Only the icon (§9 Q1)~~ — resolved, see §4.1 | 1 ticket |

Worth being explicit: Phase 1 was **not** blocked on AIP at all. It was only ever blocked on a
512×512 image — and, as §4.1 records, on a documentation error about which image was available.

**What shipped:**

| File | What it does |
|---|---|
| `src/app/manifest.ts` | `/manifest.webmanifest` — name, icons, `standalone`, theme colour, `start_url: /dashboard`. |
| `public/sw.js` | Hand-written service worker. Cache-first for content-hashed static assets, network-first for navigations, offline-page fallback. **Never touches the API** (§6.2). |
| `src/components/pwa/ServiceWorkerRegistrar.tsx` | Registers the worker; shows a "new version available — Reload" banner instead of swapping assets under a user mid-task. |
| `src/app/offline/page.tsx` | The offline fallback the worker serves when a navigation fails with nothing cached. |
| `public/icons/*` | The four icons, generated by `scripts/generate-pwa-icons.js`. |
| `src/app/layout.tsx` | `themeColor` viewport export, `apple-touch-icon`, mounts the registrar. |

**Deliberately not included:** any API caching. That is study §11 and carries a security decision
(§9 Q4) that has not been made. The worker skips cross-origin requests, and skips same-origin
`/api/*` too so that the SWA CLI's local proxy can't sneak past the origin check.
