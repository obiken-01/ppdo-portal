...
- **`Division` enum retired** → replaced by a **configurable, per-office `divisions` table**. `users.division` (enum) → **`users.division_id INT NULL FK → divisions`**. PPDO's real divisions are now: Administrative (`ADMIN`), Sectoral Planning (`PLANNING`), Statistics/Monitoring & Evaluation (`RMED`), Fiscal Planning & Investment Programming (`FPIP`), ICT (`ICT`), Open Governance & CSO (`OG-CSO`). ⚠️ Codes corrected 2026-09-02, verified by `SELECT code, name FROM divisions` against production — this line previously read `SMED`, which was the 2026-06-24 proposal and the v1.7 dev value, not what production runs. **Codes are configuration, not constants**: they live in the `divisions` table and are editable in the division config page, so query them rather than trusting any list written down here.
