...
| 10 | Access control: PPDO users (`users.office_id` = null) manage all offices; non-PPDO office users (`users.office_id` set) access only their own office. Office users are Staff. **⚠️ v1.2 update:** the `Observer` role and the `Division` enum were **retired**; `users.division` (enum) → **`users.division_id INT NULL FK → divisions`** (a configurable per-office table that also carries the permission flags; `PermissionGroup`/`GroupId` deleted). Current model: **`docs/v1.2/Allocation_Requirements.md` §5–6** (the v1.1 `User_Roles_Permissions.md` now opens with a supersession banner). |
| 10a | **v1.2 — WFP is per-division.** `wfp_records` uniqueness became `(aip_record_id, office_id, division_id)` and finalize/unlock are per-division. New v1.2 tables (`divisions`, `budget_ceilings`, `division_allocations`, `program_divisions`) are documented in `docs/v1.2/Allocation_Requirements.md` §5, not here. |
