...
function FlagBadges({ division }: { division: DivisionResponse }) {
  ...
  if (active.length === 0) return <span className="text-slate-600 text-xs">None</span>;
  return (
    ...
    },
    {
      ...
      render: (d) => (
        <span className="text-slate-600 text-sm">
          {d.officeCode ? (
            <>
              <span className="font-mono text-xs text-slate-600 mr-1">{d.officeCode}</span>
              {d.officeName ?? "—"}
            </>
          ) : (
            d.officeName ?? "—"
          ...
          <div className="space-y-4">
            ...
            <div>
              <label className="block text-xs font-medium text-slate-600 mb-1">
                Short Code <span className="font-normal text-slate-600">(optional)</span>
              </label>
              <input
                value={form.code}
