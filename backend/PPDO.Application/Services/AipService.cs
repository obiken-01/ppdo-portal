...
public sealed class AipService : IAipService
{
    ...
    //
    // A program/project row that also carries its own amounts (e.g. the Provincial Legal
    // Office's "1000-000-1-01-011-004" program) is materialized here as a synthetic project
    // and/or activity — IsSynthetic = true — rather than as new columns on AipProgram/AipProject.
    ...
    private static List<AipProgram> BuildPrograms(
        ...
        progDtos.Select(progDto =>
        {
            ...
            return new AipProgram { RefCode = progDto.RefCode, Name = progDto.Name, Projects = projects };
        }).ToList();
    ...
    private static AipActivityDto MapActivityToDto(AipActivity a) => new(
        ...
        a.Ps, a.Mooe, a.Co, a.Total, a.CcAdaptation, a.CcMitigation, a.CcTypologyCode, a.IsSynthetic);
}
